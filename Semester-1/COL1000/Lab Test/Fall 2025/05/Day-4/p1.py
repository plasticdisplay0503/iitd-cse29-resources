...
# This file implements a caller function for the csvtool utility (replace_text).
...
if __name__ == "__main__":
    try:
        ...
        for command in Input:
            ...
            # --- File Preview Logic (for replace_text) ---
            try:
                parts = shlex.split(command)
                if "-replace" in parts:
                    # According to: -replace <src> <dest> <old> <new> [--ignore_case] [--cols ...]
                    idx = parts.index("-replace")
                    if len(parts) >= idx + 3:
                        output_file = parts[idx + 2] if len(parts) >= idx + 3 else None
                        if not output_file:
                            continue
        ...
        print("p1_input.txt not found. Cannot run local tests.")
    except Exception as e:
